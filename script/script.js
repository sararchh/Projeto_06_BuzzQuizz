...
function reiniciaQuizz(){
  acertos = 0;
  perguntasClicadas = 0;
  let level = document.querySelectorAll('.containerLevels');
  let respostas = document.querySelectorAll('.containerPerguntas');
  let rebelde = document.querySelectorAll('.naoeCorreta');

  for(let i = 0; i < respostas.length; i++){
    respostas[i].classList.remove('jaClicado');
  }
  for(let j = 0; j < rebelde.length; j++){
    rebelde[j].classList.remove('opacidade');
  }
  for(let l = 0; l < level.length; l++){
    level[l].classList.add('escondida');
  }
}
